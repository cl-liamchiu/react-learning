...
const ChildWithoutUseCallback: React.FC<{ onClick: () => void }> = memo(
  ({ onClick }) => {
    console.log('ChildWithoutUseCallback rendered');
    return (
      <button style={{ marginLeft: '1rem' }} onClick={onClick}>
        ChildWithoutUseCallback Button
      </button>
    );
  },
);
...
const UseCallbackExample = () => {
    ...
    <div>
      ...
      <p style={{ marginTop: '3rem' }}>
        👉 Child 元件有用 useCallback 搭配 memo，所以不會重複 render。
      </p>
      <p>
        👉 ChildWithoutUseCallback 元件沒有用 useCallback 搭配
        memo，所以每次父元件 render 都會重複 render。
      </p>
    </div>
  );
};
