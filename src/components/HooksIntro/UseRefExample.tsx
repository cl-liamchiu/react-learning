...
`
            ...
            <div style={{ display: 'flex', gap: '2rem', marginBottom: '3rem' }}>
                <div>
                    <button onClick={() => {countRef.current ++; console.log("countRef:", countRef.current);}}>增加 countRef</button>
                    <p style={{ textAlign: 'center' }}>countRef: {countRef.current}</p>
                </div>
                <div>
                    <button onClick={() => {countVar++; console.log("countVar:", countVar);}}>增加 countVar</button>
                    <p style={{ textAlign: 'center' }}>countVar: {countVar}</p>
                </div>
                <div>
                    <button onClick={() => {setCount(count + 1); console.log("countState:", count+1);}}>增加 countState</button>
